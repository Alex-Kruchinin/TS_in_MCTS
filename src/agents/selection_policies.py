...
class ProgressiveBiasUCTSelectionPolicy:
    """
    UCT with a decaying heuristic bonus.

    The heuristic is useful early, when a child has little rollout evidence.
    As child.visits grows, the heuristic term becomes smaller and the real
    MCTS statistics dominate.
...
class ThompsonSamplingSelectionPolicy:
    """
    Select children using Thompson Sampling over Beta posteriors.

    Each child stores alpha and beta values. During selection, the policy
    samples one possible success probability from each child and selects the
    child with the highest sampled value.
    """
