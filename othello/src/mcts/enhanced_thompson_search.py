...
class EnhancedThompsonSearch:
    """Enhanced Othello MCTS with Thompson tree selection.

    It shares enhanced UCT's other features, replacing UCT selection with
    ...
    def build_tree(self, root_state: OthelloState) -> EnhancedThompsonNode:
        if root_state.is_terminal():
            raise ValueError("Cannot search from a terminal state.")

        root = EnhancedThompsonNode(
            root_state,
        ...
        for _ in range(self.simulations):
            ...
            if not node.is_terminal and not node.is_fully_expanded:
                ...
                node = node.expand(
                    self._policy,
                    self._rng,
                    expansion_epsilon=effective_epsilon,
                )
    ...
    ) -> tuple[Action, EnhancedThompsonNode]:
        root = self.build_tree(root_state)
        return self.best_root_action(root), root

    def best_root_action(self, root: EnhancedThompsonNode) -> Action:
        ...
        scored: list[tuple[float, EnhancedThompsonNode]] = []
        ...
        for child in eligible:
            ...
            scored.append((combined_score, child))
    ...
    def _root_lookahead_value(
        ...
        reply_values = [
            ...
            for action in state_after_root_action.legal_actions()
        ]
    ...
    @staticmethod
    def _terminal_values(state: OthelloState) -> tuple[float, float]:
        return (
